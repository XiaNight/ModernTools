...
/// <summary>
/// Controls the JTB500 XY robot arm via TCP.
/// All public methods accept coordinates in µm (mm × 1000).
/// XY range: 0–500,000 µm (0–500 mm). Speed range: 0–800.
/// </summary>
public class JTB500Controller : IDisposable
{
    ...
    private const string MSG_WELCOME = "welcome to TOYO!\n";
    ...
    public async Task<bool> AutoRunAsync(int procedure) =>
        await Task.Run(() =>
        {
            if (!WriteCmd(MSG_CMD_AUTO_RUN + " " + procedure + MSG_END))
                return false;

            for (int i = 0; i < 60 * 2; i++)
            {
                byte status = 0;
                if (ReadOutputIO(4, ref status) && status == 0)
                    return true;
                Thread.Sleep(500);
            }
            return false;
    ...
    /// <param name="xUm">X position in µm (mm × 1000). Range: 0–500,000.</param>
    /// <param name="yUm">Y position in µm (mm × 1000). Range: 0–500,000.</param>
    /// <param name="speed">Speed 0–800.</param>
    public async Task<bool> MoveAbsAsync(int xUm, int yUm, int speed) =>
        await Task.Run(() =>
        {
            if (xUm < 0 || xUm > 500_000 || yUm < 0 || yUm > 500_000 || speed < 0 || speed > 800)
                return false;
            return WriteCmd($"{MSG_CMD_MOVE_L} {xUm} {yUm} 0 0, V={speed}{MSG_END}");
        });

    /// <param name="dxUm">Incremental X in µm. Range: ±500,000.</param>
    /// <param name="dyUm">Incremental Y in µm. Range: ±500,000.</param>
    public async Task<bool> MoveIncAsync(int dxUm, int dyUm, int speed) =>
        await Task.Run(() =>
        {
            if (dxUm < -500_000 || dxUm > 500_000 || dyUm < -500_000 || dyUm > 500_000 || speed < 0 || speed > 800)
                return false;
            return WriteCmd($"{MSG_CMD_MOVEI_L} {dxUm} {dyUm} 0 0, V={speed}{MSG_END}");
        });
    ...
    private bool ReadWelcome()
    {
        try
        {
            var buf = new byte[256];
            int n = _stream.Read(buf, 0, buf.Length);
            return Encoding.ASCII.GetString(buf, 0, n) == MSG_WELCOME;
        }
        catch { return false; }
    }
    ...
    {
        string data = null;
        return ReadCmd(MSG_CMD_VER, ref data) && !string.IsNullOrEmpty(data);
    }
    ...
    private bool ReadCmd(string cmd, ref string response)
    {
        try
        {
            var send = Encoding.ASCII.GetBytes(cmd);
