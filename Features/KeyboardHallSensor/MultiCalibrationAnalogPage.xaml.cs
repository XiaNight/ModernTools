...
{
    [PageInfo("Multi Calibration Analog", ShortName = "MCA", NavOrder = 3, Path = ["Keyboard", "Hall Effect"])]
    public partial class MultiCalibrationAnalogPage : PageBase, IKeyboardPage
    {
        ...
        public void Parse(ReadOnlyMemory<byte> bytes, DateTime time)
        {
            ...
            if (span.Length < 4 || span[1] != 0xFA || span[2] != 0x10 || span[3] != 0x07)
                return;
        ...
        private void ButtonClicked(byte row, byte index)
        {
            ...
            string message = $"Key ({row}, {index}) Analog Values:\n";
            for (int i = 0; i < keyData.segmentValues.Count; i++)
            {
                message += $"[{i}]: {keyData.segmentValues[i]}\n";
            }
            System.Windows.MessageBox.Show(message, "Key Analog Values", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Information);
        }
