...
public sealed class UsbInterface : PeripheralInterface
{
    ...
    private static string GetDeviceString(SafeFileHandle handle, Func<SafeFileHandle, IntPtr, uint, bool> getter)
    {
        ...
        try
        {
            return getter(handle, buf, 256) ? Marshal.PtrToStringUni(buf) ?? string.Empty : string.Empty;
        }
        finally
        {
    ...
    private void InitDevice(string path, bool asyncReads)
    {
        try
        {
            ...
            int readError = _handleRead.IsInvalid ? Marshal.GetLastWin32Error() : 0;

            _handleWrite = HidNative.CreateFile(path, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, IntPtr.Zero);
            int writeError = _handleWrite.IsInvalid ? Marshal.GetLastWin32Error() : 0;

            if (_handleRead.IsInvalid) throw new IOException($"ReadError={readError} ({new Win32Exception(readError).Message})");
            if (_handleWrite.IsInvalid) throw new IOException($"WriteError={writeError} ({new Win32Exception(writeError).Message})");
            ...
            _fsRead = new FileStream(_handleRead, FileAccess.Read, _cap.InputReportByteLength, isAsync: true);
            _fsWrite = new FileStream(_handleWrite, FileAccess.Write, _cap.OutputReportByteLength, isAsync: true);
    ...
    {
        return Helpers.Constants.OMNIPIDList?.Any(pid => pid == ProductInfo.PID) == true
            ? (byte)(ProductInfo.UsagePage switch
            {
                0xFF02 => 0x01,
                0xFF00 => 0x02,
                0xFF01 => 0x03,
